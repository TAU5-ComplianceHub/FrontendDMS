...
import { faFloppyDisk, faSpinner, faRotateLeft, faFolderOpen, faChevronLeft, faChevronRight, faFileCirclePlus, faArrowLeft, faSort, faCircleUser, faBell, faShareNodes, faUpload, faRotateRight, faCircleExclamation, faPen, faSave, faArrowUp, faCaretLeft, faCaretRight, faMagicWandSparkles, faInfo, faCheckCircle, faCopy, faTimesCircle, faInfoCircle } from '@fortawesome/free-solid-svg-icons';
...
const FTSCreatePageTemplate = ({ versionPreview = false }) => {
  ...
  // The Import section itself is always visible now (it no longer hides
  // when there's nothing to import); if the creator clicks Import with no
  // JRA/PTO documents selected in Work Order Action Fields, this just
  // surfaces a toast error instead of calling the backend.
  // Route: POST /api/ftsGenerate/import-jra (see fieldTemplateCreation.mjs)
  const handleImportJra = async () => {
    if (jraDocsToImport.length === 0) {
      toast.error("No JRA documents selected in the Work Order Action Fields");
      return;
    }
  ...
  return (
    <div className="file-create-container">
      ...
      {isInfoOpen && (
        <InfoJRAImportFTS setClose={() => setIsInfoOpen(false)} />
      )}
    </div>
  );
};
