...
const ModifyJRAControl = ({ onClose, data = [], onSubmit, readOnly = false }) => {
    // Note: data should be the object containing _id, hazard, unwantedEvent, subTaskStep
    ...
    // ... (Keep existing dropdown close logic) ...
    useEffect(() => {
    ...
    useEffect(() => {
        if (data) {
            ...
            setUEInput(data.unwantedEvent || "");
            // Backend model calls it 'subTaskStep', existing frontend prop might be 'control' or 'subTaskStep'
            setControlInput(data.subTaskStep || data.control || "");
        }
    }, [data]);
    ...
    const handleSubmit = async () => {
        ...
        if (!data._id) {
            toast.error("Error: JRA ID is missing.");
            return;
        }

        try {
            await axios.put(`${process.env.REACT_APP_URL}/api/riskInfo/modify-jra/${data._id}`, {
                hazard: hazardInput,
            ...
            toast.success("JRA Control modified successfully!");
            if (onSubmit) onSubmit(); // Trigger refresh in parent
            onClose();
        } catch (error) {
            console.error("Error modifying JRA:", error);
            toast.error(error.response?.data?.error || "Failed to modify JRA Control.");
        }
    };
...
export default ModifyJRAControl;