...
const SuggestControlPopup = ({ onClose, data, onSuccess }) => {
    ...
    const approve = async () => {
        ...
        try {
            ...
            if (!res.ok) throw new Error(result.error || "Failed to approve control");

            toast.success("Control approved.", { autoClose: 1500, closeButton: false });
    ...
    const decline = async () => {
        setLoading(true);
        try {
            const res = await fetch(`${process.env.REACT_APP_URL}/api/riskInfo/decline-control/${data?._id}`, {
                method: "POST",
                headers: {
                    Authorization: `Bearer ${localStorage.getItem("token")}`,
                },
            ...
            const result = await res.json();
            if (!res.ok) throw new Error(result.error || "Failed to decline control");

            toast.warn("Control declined.", { autoClose: 1500, closeButton: false });
            setTimeout(() => {
                onSuccess?.();
                onClose();
            }, 1500);
...
export default SuggestControlPopup;