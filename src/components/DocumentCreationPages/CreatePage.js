...
import { faFloppyDisk, faCheckCircle, faTimesCircle, faSpinner, faRotateLeft, faFolderOpen, faChevronLeft, faChevronRight, faFileCirclePlus, faArrowLeft, faSort, faCircleUser, faBell, faShareNodes, faUpload, faRotateRight, faCircleExclamation, faPen, faSave, faArrowUp, faCaretLeft, faCaretRight, faL, faMagicWandSparkles, faInfo, faFileImport, faDownload, faBan, faCopy } from '@fortawesome/free-solid-svg-icons';
...
const CreatePage = ({ versionPreview = false, signedOffPreview = false }) => {
  ...
  const handleRenameDraft = async (newTitle) => {
    ...
    try {
      const response = await fetch(`${process.env.REACT_APP_URL}/api/draft/rename/${activeId}`, {
        method: "PUT",
        headers: {
  ...
  const handleSaveVersion = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${process.env.REACT_APP_URL}/api/draft/versions/${activeDraftId}`,
        {
          method: "POST",
          headers: {
            "Authorization": `Bearer ${localStorage.getItem("token")}`
  ...
  const loadData = async (loadID) => {
    try {
      ...
      const loadUrl = signedOffPreview
        ? `${process.env.REACT_APP_URL}/api/fileGenDocs/signedOffProcedure/getFile/${loadID}`
        : versionPreview && versionNumber
          ? `${process.env.REACT_APP_URL}/api/draft/versions/${loadID}/${versionNumber}`
  ...
  // Whether the user has started filling in the form (used to gate the
  // "Import JRA" button). Deliberately ignores title and the authorizations
  // table (formData.rows) per the requirement — those are allowed to be set
  // before importing without hiding the button.
  const isDefaultAim = (aim) =>
    !Array.isArray(aim) ||
    (aim.length <= 1 &&
      (!aim[0]?.text || aim[0].text.trim() === "" || aim[0].text === "The aim of the document is "));

  const isDefaultScope = (scope) =>
    !Array.isArray(scope) ||
    scope.every((item) => !item?.text || item.text.trim() === "");

  const isDefaultProcedureRows = (rows) =>
    !Array.isArray(rows) ||
    (rows.length <= 1 &&
      !rows[0]?.mainStep &&
      !rows[0]?.SubStep &&
  ...
  const isFormUntouched = () => {
    ...
    const emptyArrayFields = [
      "abbrRows", "termRows", "hazardsControls", "chapters", "references",
      "PPEItems", "HandTools", "Equipment", "MobileMachine", "Materials",
      "pictures", "supportingDocuments"
    ];

    const usedArraysEmpty = [
      usedAbbrCodes, usedTermCodes, usedPPEOptions, usedHandTools,
      usedEquipment, usedMobileMachine, usedMaterials
    ].every((arr) => Array.isArray(arr) && arr.length === 0);

    return (
      isDefaultAim(fd.aim) &&
      isDefaultScope(fd.scope) &&
      isDefaultProcedureRows(fd.procedureRows) &&
      emptyArrayFields.every((key) => Array.isArray(fd[key]) && fd[key].length === 0) &&
      usedArraysEmpty &&
      (!fd.reviewDate || fd.reviewDate === 0)
    );
  };

  // Show the Import JRA button only for a brand-new, untouched draft that
  // hasn't already been imported into:
  //  1. Not a loaded draft (loadedID is only set once a draft is loaded)
  //  2. The user hasn't already imported a JRA (isJRA)
  //  3. The user hasn't started filling in the form (title/authorizations excluded)
  const canShowImportJRA = !loadedID && !isJRA && isFormUntouched();
  ...
  const rejectDraft = async (message) => {
    ...
    try {
      const response = await fetch(`${process.env.REACT_APP_URL}/api/documentApprovals/reject-draft-proc`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          "Authorization": `Bearer ${localStorage.getItem("token")}`
  ...
  const resolveWithdrawnDraft = async (reshare, options = {}) => {
    ...
    try {
      const response = await fetch(`${process.env.REACT_APP_URL}/api/draft/resolveWithdrawn/${loadedIDRef.current}`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          "Authorization": `Bearer ${localStorage.getItem("token")}`
  ...
  const resolveRejectedDraft = async () => {
    try {
      const response = await fetch(`${process.env.REACT_APP_URL}/api/draft/resolveRejected/${loadedIDRef.current}`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          "Authorization": `Bearer ${localStorage.getItem("token")}`
      ...
      <div className="main-box-create">
        <div className="top-section-create-page">
          <div className="icons-container-create-page">
            ...
            {!readOnly && canShowImportJRA && (<div className="burger-menu-icon-risk-create-page-1">
          ...
          <TopBarDD refreshable={true} canIn={canIn} access={access} menu={"1"} create={true} loadOfflineDraft={loadOfflineData} onHome={handleHomeNav} />
        </div>
        ...
        <div className={`scrollable-box`} ref={scrollBoxRef}>
          ...
          {(!signedOffPreview && !versionPreview && !isViewer && readOnly && !inApproval && !inReview && !isRejected) && (<div className="input-row">
            <div className={`input-box-aim-cp`} style={{ marginBottom: "10px", background: "#CB6F6F", color: "white", fontWeight: "bold" }}>
        ...
        {isRenamePopupOpen && (
          <RenameDraftPopup
            ...
            siblingDraftsRoute={`${process.env.REACT_APP_URL}/api/draft/drafts/${userID}`}
            titleField="title"
          />
        )}
