...
const OnlineTrainingCreateProfileLink = ({
    onClose,
    studentName = '',
    studentEmail = '',
    profileLink = '',
    profileId = '',
    apiBase = `${process.env.REACT_APP_URL}/api/onlineTrainingStudentManagement`
}) => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const res = await fetch(
                `${process.env.REACT_APP_URL}/api/onlineTrainingStudentManagement/shareLink/${profileId}`,
                {
                    method: 'POST',
    ...
    useEffect(() => {
        ...
        (async () => {
            ...
            const url = `${apiBase}/getStudentLink/${encodeURIComponent(
                profileId
            ...
            <div className="create-visitor-profile-page-overlay">
                <div className="create-visitor-profile-page-popup-right-3">
                    <div className="create-visitor-profile-page-popup-header-right">
                        <h2>Share Student Profile</h2>
                        <button className="review-date-close" onClick={onClose} title="Close Popup">×</button>
                    </div>

                    <div className="cpl-content">
                        <div className="cpl-panel">
                            <div className="cpl-info-title">Student Information:</div>

                            <div className="cpl-info-line">
                                <span className="cpl-info-label">Student Name:</span>&nbsp;
                                <span className="cpl-info-value">{studentName || '-'}</span>
                            </div>

                            <div className="cpl-info-line">
                                <span className="cpl-info-label">Email:</span>&nbsp;
                                {studentEmail ? (
                                    <a className="cpl-info-link" href={`mailto:${studentEmail}`}>{studentEmail}</a>
                                ) : <span className="cpl-info-value">-</span>}
                            </div>

                            <div className="cpl-link-title">Student Profile Link:</div>

                            <div className="cpl-link-row">
                                <input
                                    ref={inputRef}
                                    className="cpl-link-input"
                                    placeholder={initializing ? 'Fetching link…' : 'Student Profile Link'}
                                    value={link}
...
export default OnlineTrainingCreateProfileLink;
