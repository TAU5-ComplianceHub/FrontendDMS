...
const OnlineTrainingCreateBatchProfiles = ({ onClose, openExcel, refresh }) => {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            const url = `${base}/api/onlineTrainingStudentManagement/batchCreateStudents`;
            ...
            toast.success(`Students imported successfully. Inserted: ${res?.data?.inserted ?? rows.length}`);
            refresh();
            onClose?.();
        } catch (err) {
            ...
            if (status === 400 && Array.isArray(data?.details)) {
                ...
                a.href = url;
                a.download = `student_batch_errors_${new Date().toISOString().slice(0, 10)}.txt`;
                document.body.appendChild(a);
                a.click();
                a.remove();
                URL.revokeObjectURL(url);

                toast.error(data?.error || 'Some students failed to import. A text file with details has been downloaded.');
                return;
            }
                ...
                <div className="create-visitor-profile-page-popup-right-batch">
                    ...
                    <div className="cbp-body-wrapper">
                        <p className="cbp-hint">
                            <strong>To add multiple student profiles, you can choose one of the following two options:</strong><br />
...
export default OnlineTrainingCreateBatchProfiles;
